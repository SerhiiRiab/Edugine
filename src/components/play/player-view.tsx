...
type Phase = 'nickname' | 'waiting' | 'playing' | 'finished'
...
export function PlayerView({ session, items }: Props) {
  ...
  // Always-current ref to the timeout handler to avoid stale closures in intervals
  ...
  // ── Timer ────────────────────────────────────────────────────────────────
  // Keep timeout handler ref current so the interval never captures a stale closure
  ...
  async function handleJoin(e: React.FormEvent) {
    ...
    // Check if session already has a participant (one player per session)
    ...
    // Track presence so host sees the player
  ...
  const handleSwipeInternal = useCallback((swipedRight: boolean, isTimeout = false) => {
    ...
    if (nextIndex >= items.length) {
      // Small delay to show the result flash before game over
  ...
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [currentCardIndex, items, session.id, nickname])
  ...
  function finishGame() {
    ...
    // Broadcast game_complete to host
    const stored = (() => {
      try { return JSON.parse(localStorage.getItem(`participant_${session.id}`) ?? '{}') } catch { return {} }
    })()
    channelRef.current?.send({
      ...
      {phase === 'playing' && (
        <div className="flex-1 flex flex-col">
          ...
          <div className="flex-1 flex flex-col items-center justify-center px-4 py-2 relative">
            ...
            {/* Card with AnimatePresence for enter/exit */}
              ...
              <AnimatePresence mode="wait">
                {currentCardIndex < items.length && (
                  <motion.div
                    ...
                    transition={{ duration: 0.2 }}
                  >
                    <SwipeCard item={items[currentCardIndex]} onSwipe={handleSwipe} />
                  </motion.div>
                )}
              </AnimatePresence>
            </div>

            {/* Button fallback (also accessible) */}
