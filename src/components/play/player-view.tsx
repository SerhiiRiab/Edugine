...
import type { ElevatorPitchState, ElevatorPitchItem } from '@/lib/mechanics/elevator-pitch/types'
...
export function PlayerView({ session, lesson }: Props) {
  ...
  // ── Correct the Mistake shared: fetch from DB on reconnect ──────────────────
  useEffect(() => {
    if (phase !== 'playing' || currentMechanicId !== 'correct_the_mistake' || currentActivity?.mode !== 'shared' || ctmSharedState) return
    const supabase = createClient()
    supabase
      .from('shared_activity_state')
      .select('state')
      ...
      .single()
      .then(({ data }) => {
        if (data?.state && 'fixes' in (data.state as Record<string, unknown>)) {
          setCtmSharedState(data.state as unknown as CorrectTheMistakeSharedState)
        }
      })
  // eslint-disable-next-line react-hooks/exhaustive-deps
  ...
  useEffect(() => {
    if (phase !== 'playing' || currentMechanicId !== 'debate_roulette' || debateRouletteState) return
    const supabase = createClient()
    supabase
      .from('shared_activity_state')
      .select('state')
      ...
      .single()
      .then(({ data }) => {
        if (data?.state && 'topics' in (data.state as Record<string, unknown>)) {
          setDebateRouletteState(data.state as unknown as DebateRouletteState)
        }
      })
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [phase, currentActivityIndex, currentMechanicId, debateRouletteState])

  // ── Hidden Role: fetch from DB on reconnect ───────────────────────────────────
  useEffect(() => {
    if (phase !== 'playing' || currentMechanicId !== 'hidden_role' || hiddenRoleState) return
    const supabase = createClient()
    supabase
      .from('shared_activity_state')
      .select('state')
  ...
  // ── Drama Event: fetch from DB on reconnect ───────────────────────────────────
  useEffect(() => {
    if (phase !== 'playing' || currentMechanicId !== 'drama_event' || dramaEventState) return
    const supabase = createClient()
    supabase
      .from('shared_activity_state')
      .select('state')
      ...
      .single()
      .then(({ data }) => {
        if (data?.state && 'eventHistory' in (data.state as Record<string, unknown>)) {
          setDramaEventState(data.state as unknown as DramaEventState)
        }
      })
  // eslint-disable-next-line react-hooks/exhaustive-deps
  ...
  // ── Elevator Pitch: fetch from DB on reconnect ────────────────────────────────
  useEffect(() => {
    if (phase !== 'playing' || currentMechanicId !== 'elevator_pitch' || elevatorPitchState) return
    const supabase = createClient()
    supabase
      .from('shared_activity_state')
      .select('state')
  ...
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [phase, currentActivityIndex, currentMechanicId, elevatorPitchState])

  // ── Jigsaw Reading: fetch from DB on reconnect ────────────────────────────────
  useEffect(() => {
    if (phase !== 'playing' || currentMechanicId !== 'jigsaw_reading' || jigsawReadingState) return
    const supabase = createClient()
    supabase
      .from('shared_activity_state')
      .select('state')
      ...
      .then(({ data }) => {
        if (data?.state && 'phase' in (data.state as Record<string, unknown>)) {
          setJigsawReadingState(data.state as unknown as JigsawReadingState)
        }
      })
  // eslint-disable-next-line react-hooks/exhaustive-deps
  ...
  // ── Lesson Board: fetch from DB on reconnect ─────────────────────────────────
  useEffect(() => {
    if (phase !== 'playing' || currentMechanicId !== 'lesson_board' || lessonBoardState) return
    const supabase = createClient()
    supabase
      .from('shared_activity_state')
      .select('state')
      ...
      .single()
      .then(({ data }) => {
        if (data?.state && 'snapshot' in (data.state as Record<string, unknown>)) {
          setLessonBoardState(data.state as unknown as LessonBoardState)
        }
      })
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [phase, currentActivityIndex, currentMechanicId, lessonBoardState])
  ...
  useEffect(() => {
    ...
    supabase
      ...
      .single()
      .then(({ data }) => {
        if (data?.state) setSpeedDebateState(data.state as unknown as SpeedDebateState)
      })
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [phase, currentActivityIndex, currentMechanicId, speedDebateState])

  // ── Speaking Challenge: fetch from DB on reconnect ───────────────────────────
  useEffect(() => {
    if (phase !== 'playing' || currentMechanicId !== 'speaking_challenge' || speakingChallengeState) return
    const supabase = createClient()
    supabase
      .from('shared_activity_state')
      .select('state')
      ...
      .single()
      .then(({ data }) => {
        if (data?.state) setSpeakingChallengeState(data.state as unknown as SpeakingChallengeState)
      })
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [phase, currentActivityIndex, currentMechanicId, speakingChallengeState])
  ...
  useEffect(() => {
    ...
    channel
      ...
      .on('broadcast', { event: 'game_started' }, ({ payload }) => {
        ...
        setDramaEventState(p.dramaEventState ?? null)
        ...
        setLessonBoardState(p.lessonBoardState ?? null)
      ...
      .on('broadcast', { event: 'elevator_pitch_state_update' }, ({ payload }) => {
        ...
        if (p.state) setElevatorPitchState(p.state)
      })
        ...
        <>
          ...
          {currentMechanicId === 'speed_debate' && participantId && (
            speedDebateState
              ...
              : (
                <div className="flex-1 flex items-center justify-center p-6">
                  <p className="text-slate-400 animate-pulse">Loading debate…</p>
                </div>
              )
          )}

          {/* Speaking Challenge */}
          {currentMechanicId === 'speaking_challenge' && participantId && (
            speakingChallengeState
              ...
              : (
                <div className="flex-1 flex items-center justify-center p-6">
                  <p className="text-slate-400 animate-pulse">Loading challenge…</p>
                </div>
              )
          )}
          ...
          {/* Debate Roulette */}
          {currentMechanicId === 'debate_roulette' && participantId && (
            debateRouletteState
              ? (
              ...
              : (
                <div className="flex-1 flex items-center justify-center p-6">
                  <p className="text-slate-400 animate-pulse">Loading roulette…</p>
                </div>
              )
          )}

          {/* Hidden Role */}
          {currentMechanicId === 'hidden_role' && participantId && (
            hiddenRoleState
              ? (
                <HiddenRolePlayerPanel
                  participantId={participantId}
                  state={hiddenRoleState}
                  items={currentItems as unknown as HiddenRoleItem[]}
                  participants={waitingParticipants}
                  channelRef={channelRef}
                />
              )
          ...
          {currentMechanicId === 'drama_event' && (
            dramaEventState
              ...
              : (
                <div className="flex-1 flex items-center justify-center p-6">
                  <p className="text-slate-400 animate-pulse">Loading scenario…</p>
                </div>
              )
          )}

          {/* Elevator Pitch */}
          {currentMechanicId === 'elevator_pitch' && participantId && (
            elevatorPitchState
              ? (
                <ElevatorPitchPlayerPanel
                  ...
                  items={currentItems as unknown as ElevatorPitchItem[]}
                  participants={waitingParticipants}
                  channelRef={channelRef}
                  activityIndex={currentActivityIndex}
                />
          ...
          {/* Jigsaw Reading */}
          {currentMechanicId === 'jigsaw_reading' && participantId && (
            jigsawReadingState
              ? (
