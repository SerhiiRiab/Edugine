...
import type { PublicLesson, PublicProgram } from './page'
...
function LessonsSection({ lessons }: { lessons: PublicLesson[] }) {
...
function ProgramsSection({ programs }: { programs: PublicProgram[] }) {
  const [search, setSearch] = useState('')
...
export function PublicCatalogContent({
  lessons, programs, initialTab = 'lessons',
}: {
  lessons: PublicLesson[]
  programs: PublicProgram[]
  initialTab?: 'lessons' | 'programs'
}) {
  const [tab, setTab] = useState<'lessons' | 'programs'>(initialTab)
  ...
  return (
    <main className="flex-1 max-w-6xl mx-auto w-full px-4 md:px-6 py-10">
      ...
      <div className="mb-6">
        <div className="flex items-center gap-2 mb-2">
          ...
          <h1 className="text-2xl font-extrabold text-slate-800">
            {tab === 'lessons' ? 'Public Lessons' : 'Public Programs'}
          </h1>
        </div>
        <p className="text-slate-400 text-sm">
          {tab === 'lessons'
            ? 'Free interactive lessons created by tutors'
            : 'Ready-made lesson sequences shared by tutors'}
        </p>
      </div>
      ...
      <div className="flex items-center gap-1 mb-8 border-b border-slate-200">
        <button
          ...
          className={`px-4 py-2.5 text-sm font-semibold border-b-2 -mb-px transition-colors ${
            tab === 'lessons' ? 'border-violet-600 text-violet-700' : 'border-transparent text-slate-400 hover:text-slate-600'
          }`}
        >
          Lessons
        </button>
        <button
          ...
          className={`px-4 py-2.5 text-sm font-semibold border-b-2 -mb-px transition-colors ${
            tab === 'programs' ? 'border-violet-600 text-violet-700' : 'border-transparent text-slate-400 hover:text-slate-600'
          }`}
        >
          Programs
        </button>
      ...
      {tab === 'lessons' ? <LessonsSection lessons={lessons} /> : <ProgramsSection programs={programs} />}
    </main>
  )
}
