...
import {
  ...
  Loader2, ClipboardList, Layers, Rocket,
} from 'lucide-react'
import {
...
import {
  updateContentSet, updateContentItem, createContentItem, deleteContentItem,
  reorderContentItems, bulkCreateContentItems,
} from '@/lib/actions/content-sets'
import { createSession } from '@/lib/actions/sessions'
import { BulkImportModal } from '@/components/tutor/bulk-import-modal'
import { wordCardsDefinition } from './index'
import type { ContentEditorProps } from '@/lib/mechanics/types'
import type { WordCardsItem } from './types'
...
function rawToCard(item: RawItem): EditorCard {
...
function SortableCardRow({
  ...
  item: EditorCard
  index: number
  onUpdate: (id: string, patch: Partial<EditorCard>) => void
  ...
  return (
    ...
    >
      ...
      <input
        ...
        placeholder="Front (word / sentence / question)"
      ...
      <input
        ...
        placeholder="Back (definition / translation / answer)"
        className="flex-1 text-sm text-slate-800 bg-slate-50 rounded-lg
          border border-slate-200 focus:border-sky-400 focus:ring-2 focus:ring-sky-100
          outline-none px-3 py-2 transition-colors placeholder:text-slate-300"
      />
...
export function WordCardsContentEditor(_props: ContentEditorProps<WordCardsItem>) {
  return null
}
...
export function WordCardsContentEditorPage({ set, initialItems }: PageProps) {
  const router = useRouter()
  const [title, setTitle] = useState(set.title)
  const [editingTitle, setEditingTitle] = useState(false)
  const [items, setItems] = useState<EditorCard[]>(initialItems.map(rawToCard))
  ...
  function flushItem(item: EditorCard) {
  ...
  function updateItem(id: string, patch: Partial<EditorCard>) {
  ...
  async function handleBulkImport(rows: Record<string, unknown>[]) {
    try {
      ...
      const newItems = created.map(c => rawToCard(c as RawItem))
      setItems(prev => [...prev, ...newItems])
      setShowBulkImport(false)
      toast.success(`Added ${rows.length} card${rows.length !== 1 ? 's' : ''}`)
    } catch {
      toast.error('Bulk import failed')
    }
  }

  const canPlay = items.length >= 1
      ...
      <div className="bg-white border-b border-slate-200 px-4 sm:px-6 py-3 sticky top-0 z-10">
        <div className="max-w-3xl mx-auto flex items-center gap-4">
          ...
          <span className="inline-flex items-center gap-1 text-xs font-semibold px-2 py-1 rounded-full border
            ...
            <Layers className="w-3 h-3" />Word Cards
          </span>
          ...
          <button
            ...
            title={canPlay ? 'Start a live session' : 'Add at least 1 card'}
            onClick={() => startSessionTransition(() => createSession(set.id, undefined, 'individual'))}
      ...
      <div className="max-w-3xl mx-auto px-4 sm:px-6 py-8 space-y-4">
        <div className="flex items-center justify-between">
          <p className="text-sm text-slate-500">
            ...
            {!canPlay && <span className="ml-2 text-amber-500 font-medium">— add at least 1 to play</span>}
          </p>
          <button
            type="button"
          ...
          <SortableContext items={items.map(i => i.id)} strategy={verticalListSortingStrategy}>
            <div className="space-y-2">
              {items.map((item, index) => (
                <SortableCardRow key={item.id} item={item} index={index} onUpdate={updateItem} onDelete={handleDeleteItem} />
              ))}
            </div>
          </SortableContext>
        </DndContext>
        ...
        >
          {addingCard ? (<><Loader2 className="w-4 h-4 animate-spin" /> Adding...</>) : (<><Plus className="w-4 h-4" /> Add card</>)}
        </button>
      </div>

      {showBulkImport && wordCardsDefinition.bulkImport && (
        <BulkImportModal
          config={wordCardsDefinition.bulkImport}
