...
export function TrueFalseHostComponent(_props: MechanicHostProps<TrueFalseState>) {
  return null
}
...
export interface TrueFalseVoteHostPanelProps {
  voteState: VoteState
...
export function TrueFalseVoteHostPanel({
  voteState,
  items,
  participants,
  isLastActivity,
  ...
  const totalVoted = trueCount + falseCount
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-5 space-y-4">
        ...
        <div className="space-y-2">
          <VoteBar
            ...
            color="emerald"
            isCorrect={voteState.revealed ? item?.isTrue === true : undefined}
          />
          <VoteBar
            label="False ✗"
            count={falseCount}
            total={totalParticipants}
            color="red"
            isCorrect={voteState.revealed ? item?.isTrue === false : undefined}
          />
        </div>
          ...
          <div className="space-y-2">
            <div className="flex items-center justify-center gap-2 py-2 rounded-xl bg-emerald-50 border border-emerald-200">
              ...
              <span className="text-sm font-semibold text-emerald-700">
                Correct: {item?.isTrue ? 'True ✓' : 'False ✗'}
              </span>
            </div>
            {!isLastQuestion ? (
              <button
      ...
      {/* Navigation (non-last question is handled above, show end-lesson option always) */}
      {!isLastQuestion && (
...
function VoteBar({
  label, count, total, color, isCorrect,
}: {
  label: string
  count: number
  total: number
  ...
  return (
    <div className="flex items-center gap-3">
      <span className={`w-24 text-sm font-semibold shrink-0 ${textColor}`}>{label}</span>
      ...
      <span className="w-8 text-right text-sm font-bold text-slate-700 tabular-nums">{count}</span>
